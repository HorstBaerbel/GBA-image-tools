...
namespace Dxtv
{
    ...
    IWRAM_FUNC void UnCompWrite16bit<240>(const uint32_t *data, uint32_t *dst, const uint32_t *prevSrc, uint32_t width, uint32_t height)
    {
        ...
        for (uint32_t by = 0; by < height / Video::DxtvConstants::BLOCK_MAX_DIM; ++by)
        {
            ...
            auto currPtr32 = dst + by * LineStride32 * Video::DxtvConstants::BLOCK_MAX_DIM;
            auto prevPtr32 = prevSrc == nullptr ? nullptr : prevSrc + by * LineStride32 * Video::DxtvConstants::BLOCK_MAX_DIM;
