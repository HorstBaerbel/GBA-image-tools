...
extern "C" const uint32_t C2C3_ModeThird_5bit[1024];
...
extern "C" const uint8_t C2_ModeHalf_5bit[1024];
...
namespace DXT
{
    ...
    FORCEINLINE auto getBlockColors(const uint16_t *data, uint16_t *colors) -> const uint16_t *
    {
        ...
        else
        {
            ...
            //*c2c3Ptr = (b << 10) | (g << 5) | r;
            *c2c3Ptr = (static_cast<uint32_t>(C2_ModeHalf_5bit[b]) << 10) | (static_cast<uint32_t>(C2_ModeHalf_5bit[g]) << 5) | static_cast<uint32_t>(C2_ModeHalf_5bit[r]);
        }
        return data;
    }
}
