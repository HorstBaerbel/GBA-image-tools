...
namespace IO::Vid2h
{
    ...
    struct FileHeader
    {
        ...
        uint8_t videoColorMapEntries : 8;      // Number of color table entries
        uint32_t videoNrOfColorMapFrames : 24; // Number of colormap frames (must not be the same as video frames)
        uint8_t videoSwappedRedBlue : 8;       // If != 0 red and blue color channels are swapped
        uint32_t videoMemoryNeeded : 24;       // Max. intermediate memory needed to decompress an image frame. 0 if data can be directly written to destination (single compression stage)
