...
namespace Compression
{
    ...
    auto encodeLZ4(const std::vector<uint8_t> &data, bool vramCompatible = false) -> std::vector<uint8_t>;

    /// @brief Decompress input data using LZ4 variant 40h
    auto decodeLZ4(const std::vector<uint8_t> &data, bool vramCompatible = false) -> std::vector<uint8_t>;
}
