...
namespace Image
{
    ...
    /// Indices are returned in OUT_TYPE units
    template <typename OUT_TYPE, typename INDEX_TYPE = uint32_t>
    std::pair<std::vector<OUT_TYPE>, std::vector<INDEX_TYPE>> combineRawMapData(const std::vector<Frame> &images)
    {
        ...
        return {DataHelpers::combineTo<OUT_TYPE>(temp8), DataHelpers::divideBy<INDEX_TYPE>(DataHelpers::getStartIndices<uint8_t, INDEX_TYPE>(temp8), sizeof(OUT_TYPE) / sizeof(uint8_t))};
    }
    ...
    std::pair<std::vector<OUT_TYPE>, std::vector<uint32_t>> combineRawColorMapData(const std::vector<Frame> &images)
    {
        ...
        auto startIndices = DataHelpers::getStartIndices<uint8_t, uint32_t>(temp8);
        for (auto index : startIndices)
        {
            REQUIRE(index % sizeof(OUT_TYPE) == 0, std::runtime_error, "The image pixel data size of all images must be evenly dividable by " << sizeof(OUT_TYPE) / sizeof(uint8_t));
        }
