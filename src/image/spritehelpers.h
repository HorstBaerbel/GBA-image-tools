...
namespace Image
{
    ...
    auto convertToWidth(const PixelData &data, uint32_t width, uint32_t height, uint32_t tileWidth = 8) -> PixelData;
    ...
    /// Moves from left to right first, then top to bottom.
    auto convertToTiles(const PixelData &data, uint32_t width, uint32_t height, uint32_t tileWidth = 8, uint32_t tileHeight = 8) -> PixelData;
