...
auto hashTileBlock(const std::vector<Color::XRGB8888>::const_iterator src, uint32_t columns, uint32_t rows, bool hashFlips) -> std::array<uint32_t, 4>
{
    ...
    for (int32_t y = 0; y < rows; y++)
    {
        for (int32_t x = 0; x < columns; x++, ++src0)
        {
            ...
            hash[0] += p * (2 + static_cast<uint32_t>(src0->R()));
            hash[0] += p * (263 + static_cast<uint32_t>(src0->G()));
            hash[0] += p * (521 + static_cast<uint32_t>(src0->B()));
        }
    }
    if (hashFlips)
    {
        ...
        for (int32_t y = 0; y < rows; y++)
        {
            ...
            {
                auto p = primes[pi++ % (primes.size() - 1)];
                hash[1] += p * (2 + static_cast<uint32_t>(src1->R()));
                hash[1] += p * (263 + static_cast<uint32_t>(src1->G()));
                hash[1] += p * (521 + static_cast<uint32_t>(src1->B()));
            }
        }
        ...
        for (int32_t y = rows - 1; y >= 0; y--)
        {
            ...
            {
                auto p = primes[pi++ % (primes.size() - 1)];
                hash[2] += p * (2 + static_cast<uint32_t>(src2->R()));
                hash[2] += p * (263 + static_cast<uint32_t>(src2->G()));
                hash[2] += p * (521 + static_cast<uint32_t>(src2->B()));
            }
        }
        ...
        for (int32_t y = 0; y < rows; y++)
        {
            ...
            {
                auto p = primes[pi++ % (primes.size() - 1)];
                hash[3] += p * (2 + static_cast<uint32_t>(src3->R()));
                hash[3] += p * (263 + static_cast<uint32_t>(src3->G()));
                hash[3] += p * (521 + static_cast<uint32_t>(src3->B()));
            }
        }
    }
    return hash;
...
auto buildUniqueTileMap(const std::vector<Color::XRGB8888> &data, uint32_t width, uint32_t height, bool detectFlips, uint32_t tileWidth, uint32_t tileHeight) -> std::pair<std::vector<uint16_t>, std::vector<Color::XRGB8888>>
{
    REQUIRE(data.size() == width * height, std::runtime_error, "Data size must be == width * height");
    REQUIRE(tileWidth % 8 == 0 && tileHeight % 8 == 0, std::runtime_error, "Tile width and height must be divisible by 8");
    REQUIRE(width % 8 == 0 && height % 8 == 0, std::runtime_error, "Width and height must be divisible by 8");
